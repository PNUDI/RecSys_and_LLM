...
from langchain.schema import BaseMemory, BaseRetriever, Document
...
class AvatarMemory(BaseMemory):
    ...
    def get_completion(self, prompt, llm="gpt-3.5-turbo", temperature=0):
        ...
        while response == "":
            try:
                    ...
                    if (
                        ...
                        if vars.lock.acquire(False):
                            print(
                                ...
                                vars.lock,
                                (start_time - vars.global_start_time)
                                // vars.global_interval,
                                vars.global_interval,
                                vars.global_steps,
                            )
                            print(
                                "\nMemory Start Identifier",
                                start_time,
                ...
                print("===================================")
                print(
                    f'{response["usage"]["total_tokens"]} = {response["usage"]["prompt_tokens"]} + {response["usage"]["completion_tokens"]} tokens counted by the OpenAI API.'
                )
    ...
    def _user_taste_reflection(self, last_k: int = 10) -> List[str]:
        ...
        result = self.get_completion(
            prompt=prompt_filled, llm="gpt-3.5-turbo", temperature=0.2
        )
        print(result)
        return result
    ...
    def pause_to_reflect_satisfaction(
        self, now: Optional[datetime.datetime] = None
    ) -> List[str]:
        """Reflect on recent observations and generate 'insights'."""
