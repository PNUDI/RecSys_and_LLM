...
class Avatar(abstract_avatar):
    ...
    def parse_init_property(self, init_property):
        ...
        self.high_rating = init_property["reason"]
    ...
    def init_memory(self):
        ...
        cprint(
            f"Avatar {self.avatar_id} is initialized with memory",
            color="green",
            attrs=["bold"],
        )
        ...
        while response == "":
            try:
                ...
                if self.use_wandb:  # whether to use wandb
                    if (
                        start_time - vars.global_start_time
                    ) // vars.global_interval > vars.global_steps:
                        print(
                            "\nStart Identifier",
                            start_time,
                            vars.global_start_time,
                            (start_time - vars.global_start_time),
                            vars.global_steps,
                        )
                        if vars.lock.acquire(False):
                            ...
                            wandb.log(
                                data={
                                    "Real-time Traffic": vars.global_k_tokens
                                    - vars.global_last_tokens_record,
                            ...
                            print(
                                ...
                                time.time(),
                                vars.global_start_time,
                                (time.time() - vars.global_start_time),
                                vars.global_steps,
                            )
                ...
                print(
                    f"User {self.avatar_id} used {k_tokens} tokens from {l_start} to {l_end}"
                )
    ...
    def make_next_decision(self, remember=False, current_page=None):
        ...
        formated_relevant_memories = self.memory.format_memories_detail(
            relevant_memories
        )
        sys_prompt = (
    ...
    def response_to_question(self, question, remember=False):
        ...
        sys_prompt = (
            f"You excel at role-playing. Picture yourself as user {self.avatar_id} who has just finished exploring a movie recommendation system. You have the following social traits:"
            + f"\nYour activity trait is described as: {self.activity_dsc}"
        ...
        if remember:
            self.memory.add_memory(
                f"I was asked '{question}', and I responsed: '{response}'",
    ...
    def reaction_to_recommended_items(self, recommended_items_str, block_name):
        """
        Summarize the feelings of the avatar for recommended item list.
        """
        try:
        ...
        prompt = (
            "#### Recommended Block #### \n"
            + f"Block theme: {block_name}\n"
            + recommended_items_str
            + "\nPlease review all the movies in the ## Recommended Block ## and provide explanations."
            ...
            + "\nLastly, evaluate how well each movie fits the theme of this recommendation block. Consider whether the movie clearly matches the intention of this block’s recommendation strategy. Use this format:"
            + "\nMOVIE: [movie name]; BLOCK-FIT: [yes or no]; REASON: [brief reason]"
        ...
        all_movies = ", ".join(
            [movie_title.strip(";") for movie_title, align, reason in match2]
        )
        watched_movies = [
            movie_title.strip(";") for movie_title, rating, feeling in match1
        ]
        watched_movies_ratings = [
            rating.strip(";") for movie_title, rating, feeling in match1
        ]
        like_movies = [
            movie_title.strip(";")
        ...
        dislike_movies = [
            movie_title.strip(";")
            for movie_title, rating, feeling in match1
            if (int(rating.strip(";")) < 4)
        ]
        dislike_movies.extend(
            [
                movie_title.strip(";")
        ...
        self.memory.add_memory(
            f"The recommender recommended the following movies to me on block {block_name}: {all_movies}, among them, I watched {watched_movies} and rate them {watched_movies_ratings} respectively. I dislike the rest movies: {dislike_movies}.",
            now=datetime.datetime.now(),
        )
