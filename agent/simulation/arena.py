...
class Arena(abstract_arena):
    ...
    def load_additional_info(self):
        ...
        self.user_profile_csv = pd.read_csv(
            f"datasets/{self.dataset}/raw_data/agg_top_25.csv"
        )
        ...
        if self.add_advert:
            ...
            advert_pool = pd.read_pickle(
                f"datasets/{self.dataset}/simulation/advertisement_review.pkl"
            )
            advert_dict = {
                "all": {
                    **advert_pool["pop_high_rating"],
                    **advert_pool["pop_low_rating"],
                    **advert_pool["unpop_high_rating"],
                    **advert_pool["unpop_low_rating"],
                },
                "pop_high": advert_pool["pop_high_rating"],
                "pop_low": advert_pool["pop_low_rating"],
                "unpop_high": advert_pool["unpop_high_rating"],
                "unpop_low": advert_pool["unpop_low_rating"],
            }
    ...
    def initialize_all_avatars(self):
        ...
        for avatar_id in self.simulated_avatars_id:
            self.avatars[avatar_id] = Avatar(
                self.args,
                avatar_id,
                self.persona_df.loc[avatar_id],
                self.user_statistic.loc[avatar_id],
            )
    ...
    def page_generator(self, avatar_id):
        ...
        while (i + 1) * self.items_per_page < self.data.n_items:
            yield self.full_rankings[avatar_id][
                i * self.items_per_page : (i + 1) * self.items_per_page
            ]
            i += 1
    ...
    async def async_simulate_one_avatar(self, avatar_id, loop, executor):
        ...
        while not avatar_.exit_flag:
            ...
            id_on_page = next(
                page_generator, []
            )  # get the next page, a list of item ids
            if len(id_on_page) == 0:
                break
            movies_on_page = [
                self.movie_detail.loc[idx] for idx in id_on_page
            ]  # movie_detail.csv
            recommended_items = [
                "<- " + item.title + " ->"
            ...
            if self.add_advert:
                ...
                if not self.display_advert:
                    recommended_items[0], id_on_page, movies_on_page = (
                        self.display_only_adver_item(
                            store_path, avatar_id, i, id_on_page, movies_on_page
                        )
                    )
                else:
                    recommended_items[0], id_on_page, movies_on_page = (
                        self.display_item_with_adver(
                            store_path, avatar_id, i, id_on_page, movies_on_page
                        )
                    )
            ...
            watched_movies_contain_id = [
                ...
                for idx, (movie_title, rating, feeling) in enumerate(
                    match1[: self.items_per_page]
                )
            ]
            ...
            align_movies = [
                ...
                for idx, (movie_title, align, reason) in enumerate(
                    match2[: self.items_per_page]
                )
