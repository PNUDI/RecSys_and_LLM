...
class Arena(abstract_arena):
    ...
    def initialize_all_avatars(self):
        ...
        # )
        self.persona_df = pd.read_csv(f"datasets/{self.dataset}/simulation/persona.csv")
        self.user_statistic = pd.read_csv(
            f"datasets/{self.dataset}/simulation/user_statistic.csv", index_col=0
        )
        ...
        for avatar_id in self.simulated_avatars_id:
            self.avatars[avatar_id] = Avatar(
                self.args,
                avatar_id,
                self.persona_df.loc[int(avatar_id)],
                self.user_statistic.loc[int(avatar_id)],
            )
    ...
    def validate_all_avatars(self):
        ...
        with open(self.storage_base_path + "/validation_metrics.txt", "w") as f:
            f.write(
                f"Total simulation time: {round(time.time() - self.start_time, 2)}s\n"
            )
    ...
    async def async_validate_one_avatar(self, avatar_id, loop, executor):
        ...
        train_list, val_list, test_list = (
            self.data.train_user_list[avatar_id],
            self.data.valid_user_list[avatar_id],
            self.data.test_user_list[avatar_id],
        )
        ...
        test_observed_items = np.random.choice(
            selection_candidates, min_val, replace=False
        )
        test_unobserved_items = np.random.choice(
            unobserved_items, int(min_val * self.val_ratio), replace=False
        )
        ...
        response = await loop.run_in_executor(
            executor, avatar_.reaction_to_forced_items, recommended_items_str
        )
        ...
        like_movies = [
            (idx, movie_title.strip(";"))
            for idx, (movie_title, watch, reason) in enumerate(
                matches[: len(forced_items)]
            )
            if (watch.strip(";") == "yes" or watch.strip(";") == "Yes")
        ]
        print("like_movies", like_movies)
        like_movies_ids = [forced_items_ids[idx] for idx, movie_title in like_movies]

        pred = np.array(
            [1 if idx in like_movies_ids else 0 for idx in forced_items_ids]
        )
        true = np.array(
            [1 if idx in test_observed_items else 0 for idx in forced_items_ids]
        )
    ...
    def simulate_all_avatars(self):
        ...
        if self.execution_mode == "serial":
            ...
            t1 = time.time()
            for avatar_id in self.simulated_avatars_id:
                coro = self.async_simulate_one_avatar(avatar_id, loop, executor)
    ...
    async def async_simulate_one_avatar(self, avatar_id, loop, executor):
        ...
        recommend_result = self.block_recommendations[avatar_id]

        movies_on_block = {}
        for k, v in recommend_result.items():
            movies_info = [self.movie_detail.loc[idx] for idx in v]
            recommended_items = [
                "<- " + item.title + " ->"
            ...
            movies_on_block[k] = recommended_items

            recommended_items_str = "".join(recommended_items)
            print(k, recommended_items_str)

            # Please write down the recommended information.
            avatar_.write_log(
                f"\n=============    Recommendation Block <{k}>    ============="
            )
            for idx, movie in enumerate(movies_info):
                # if id_on_page[idx] in self.data.valid_user_list[avatar_id]:
                #     avatar_.write_log(
                #         f"== (√) {movie.title} History ratings: {round(movie.rating,2)} Summary: {movie.summary}",
                #         "blue",
                #         attrs=["bold"],
                #     )
                # else:
                ...
                #     )
                avatar_.write_log(
                    f"== {movie.title} History ratings: {round(movie.rating,2)} Summary: {movie.description}"
                )
            avatar_.write_log(
                f"=============          End Block <{k}>        =============\n"
            )
            ...
            avatar_.write_log(
                f"\n==============    Avatar {avatar_.avatar_id} Response to Block <{k}>   ============="
            )

            # @ most important Waiting for user response.
            response = await loop.run_in_executor(
                executor,
                avatar_.reaction_to_recommended_items,
            ...
            pattern3 = re.compile(r"MOVIE: (.+?); BLOCK-FIT: (.+?); REASON: (.*)")
            match3 = pattern3.findall(response)
            ...
            title_id_dict = dict(
                zip(self.movie_detail["title"], self.movie_detail["movie_id"])
            )
            # print(title_id_dict)
            watched_movies = [
                movie_title.strip(";") for movie_title, rating, feeling in match1
            ]
            watched_movies_contain_id = [
                (idx, movie_title.strip(";"), feeling.strip(";"))
                for idx, (movie_title, rating, feeling) in enumerate(match1)
            ]
            # 5 points means the movie is liked by the user.
            like_movies = [
                (idx, movie_title.strip(";"), feeling.strip(";"))
                for idx, (movie_title, rating, feeling) in enumerate(match1)
                if int(rating.strip(";")) == 5
            ]
            align_movies = [
                (idx, movie_title.strip(";"), reason.strip(";"))
                for idx, (movie_title, align, reason) in enumerate(match2)
                if (align.strip(";") == "Yes" or align.strip(";") == "yes")
            ]
            ...
            block_evaluation = {
                "evaluated_movies": [
                    {
                        "movie": movie.strip(),
                        "block_fit": fit.strip().lower(),
                        "reason": reason.strip(),
                    }
                    for movie, fit, reason in match3
                ]
            }

            user_block_dict[k] = block_evaluation
            print(block_evaluation)

            info_on_page = {}
            info_on_page["block"] = k
            info_on_page["ground_truth"] = [
                id_on_page[idx]
                for idx, movie in enumerate(movies_info)
                if id_on_page[idx] in self.data.valid_user_list[avatar_id]
            ]
            info_on_page["recommended_id"] = id_on_page
            info_on_page["recommended"] = [
                self.movie_detail["title"][idx] for idx in id_on_page
            ]
            info_on_page["align_id"] = [
                title_id_dict[title]
                for id, title, reason in align_movies
                if title in title_id_dict
            ]
            info_on_page["like_id"] = [
                title_id_dict[title]
                for id, title, reason in like_movies
                if title in title_id_dict
            ]
            info_on_page["watch_id"] = [
                title_id_dict[title]
            ...
            info_on_page["rating"] = [
                int(rating.strip(";")) for movie_title, rating, feeling in match1
            ]
            # info_on_page['reason'] = [reason.strip(';') for movie_title, rating, feeling in match1]
            info_on_page["feeling"] = [
                feeling.strip(";") for movie_title, rating, feeling in match1
            ]
            ...
            ratings = re.findall(r"RATING: (\d+);", response)
            average_rating = sum([int(rating.strip(";")) for rating in ratings]) / max(
                len(watched_movies), 1
            )
            ...
            perf = (
                len(set(new_train) & set(ground_truth)),
                len(new_train),
                len(ground_truth),
            )
        ...
        block_path = self.storage_base_path + "/block"
        if not os.path.exists(block_path):
            os.makedirs(block_path)
        with open(block_path + f"/{avatar_id}.pkl", "wb") as f:
            pickle.dump(user_block_dict, f)
    ...
    def simulate_one_avatar(self, avatar_id):
        ...
        for k, v in recommend_result.items():
            ...
            # @ most important
            response = avatar_.reaction_to_recommended_items(recommended_items_str, k)
    ...
    def parse_response(self, response):
        # pattern = re.compile(r'MOVIE:\s*(.*?)\s*WATCH:\s*(.*?)\s*REASON:\s*(.*?)\s*FEELING:\s*(.*?)\s*RATING:\s*(\d)')
        pattern = re.compile(
            r"MOVIE:\s*(.*?)\s*WATCH:\s*(.*?)\s*REASON:\s*(.*?)\s*RATING:\s*(.*?)\s*FEELING:(.*?)"
        )
        matches = re.findall(pattern, response)
        ...
        for idx, (movie_title, watch, reason, rating, feeling) in enumerate(matches):
            if (
                self.add_advert and idx == 0 and watch.strip(";") == "yes"
            ):  # If the first one has an advertisement and the user clicked on it.
                self.clicked_adverts += 1
            if watch.strip(";") == "yes":
    ...
    def display_only_adver_item(
        self, store_path, avatar_id, i, id_on_page, movies_on_page
    ):
        ...
        try:
            ...
            try:
                ...
                with open(store_path_minus_2, "r") as f3:
                    ...
                    try:
                        store_path_minus_3 = op.join(
                            store_path, f"avatar{avatar_id}_{i-3}.txt"
                        )
                        with open(store_path_minus_3, "r") as f4:
                            random_key = int(f4.read())
                    except:
                        store_path_minus_4 = op.join(
                            store_path, f"avatar{avatar_id}_{i-4}.txt"
                        )
        ...
        return (
            (
                "<- "
                + adver_information["title"]
                + " ->"
                + " <- History ratings:"
                + str(round(adver_information["rating"], 2))
                + " ->"
                + " <- Summary:"
                + adver_information["summary"]
                + " ->"
                + "\n"
            ),
            id_on_page,
    ...
    def display_item_with_adver(
        ...
        return (
            (
                ...
                + " ->"
                + " <- History ratings:"
                + str(round(random_advert["rating"], 2))
                + " ->"
                + " <- Summary:"
                + random_advert["summary"]
                + " ->"
                + "\n"
            ),
            id_on_page,
    ...
    def save_results(self):
        ...
        average_n_likes = {
            avatar_id: np.mean(n_likes) for avatar_id, n_likes in self.n_likes.items()
        }
        cprint(average_n_likes, color="green", attrs=["bold"])

        overall_n_likes = np.mean(list(average_n_likes.values()))
        cprint(
            f"\nOverall number of likes: {overall_n_likes}",
            color="green",
            attrs=["bold"],
        )
        ...
        average_ratings = {
            avatar_id: np.mean(ratings) for avatar_id, ratings in self.ratings.items()
        }
        cprint(average_ratings, color="green", attrs=["bold"])

        # @ Save average click-through rate
        average_click_rate = {
            avatar_id: len(movies) / (self.max_pages * self.items_per_page)
        ...
        cprint(
            f"\nOverall satisfaction: {overall_click_rate}",
            color="green",
            attrs=["bold"],
        ...
        total_perf = {
            ...
            for avatar_id, perf_per_page in self.perf_per_page.items()
        }
        total_recall_precision = {
            avatar_id: (perf[0] / max(perf[1], 1), perf[0] / max(perf[2], 1))
        ...
        average_precision = np.mean(
            [metrics[0] for avatar_id, metrics in total_recall_precision.items()]
        )
        average_recall = np.mean(
            [metrics[1] for avatar_id, metrics in total_recall_precision.items()]
        )
        cprint(
            f"Precision: {average_precision}  Recall: {average_recall}",
            color="green",
            attrs=["bold"],
        )
        # metrics_path = self.storage_base_path + "/metrics.txt"
        total_k_tokens = sum(
            [self.avatars[i].memory.user_k_tokens for i in range(self.n_avatars)]
        )
        ...
        if self.add_advert:
            ...
            cprint(
                f"Clicked advert: {self.clicked_adverts}", color="green", attrs=["bold"]
            )
            cprint(
                f"Advert click rate: {self.clicked_adverts/self.total_adverts}",
                color="green",
                attrs=["bold"],
            )
